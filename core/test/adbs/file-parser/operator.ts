...
import { parse } from '../../../src/adbs/file-parser/operator.js'
...
describe('parse', () => {
  it('should handle updates', async () => {
    ...
    const parsed = await firstValueFrom(of({ key: 'path', kind: 'update', hint: 'add' }).pipe(parse([parser])))
  ...
  it('should handle deletes', async () => {
    ...
    const parsed = await firstValueFrom(of({ key: 'path', kind: 'delete' }).pipe(parse([parser])))
    expect(parsed).to.deep.equal({ key: 'path', kind: 'delete' })
  })

  it('should handle moves', async () => {
    const parser = { parse: stub().resolves([]) }
    const parsed = await firstValueFrom(of({ key: 'path', kind: 'move', newKey: 'newPath' }).pipe(parse([parser])))
  ...
  it('should handle errors', async () => {
    ...
    const parsed = await firstValueFrom(
      of({ key: 'path', kind: 'update', hint: 'add' }).pipe(parse([parser]), toArray())
    )
  ...
  it('should handle multiple parsers', async () => {
    ...
    const parsed = await firstValueFrom(
      of({ key: 'path', kind: 'update', hint: 'add' }).pipe(parse([parser1, parser2]), toArray())
    )
    expect(parsed).to.deep.equal([
