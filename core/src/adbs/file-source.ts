...
export class FileSource {
  ...
  observe(documentDirectory: string): Observable<ChangeEvent<void, string>> {
    ...
    watcher.on('add', (path) =>
      subject.next({ key: join(documentDirectory, path), kind: 'update', hint: 'add' } as UpdateEvent<void, string>)
    )
    watcher.on('change', (path) =>
      subject.next({ key: join(documentDirectory, path), kind: 'update', hint: 'update' } as UpdateEvent<void, string>)
    )
    watcher.on('addDir', (path) =>
      subject.next({ key: join(documentDirectory, path), kind: 'update', hint: 'add' } as UpdateEvent<void, string>)
    )
    watcher.on('unlink', (path) =>
      subject.next({ key: join(documentDirectory, path), kind: 'delete' } as ChangeEvent<void, string>)
    )
