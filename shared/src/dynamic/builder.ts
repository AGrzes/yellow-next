...
export const manyToMany =
  (target: string, name?: string, reverseName?: string, predicate?: string) => (b: ClassBuilder | PropertyBuilder) => {
    name = name || _.camelCase(target)
    reverseName = reverseName || _.camelCase(b.className)
    return b.property(name).manyToMany().target(target).reverse(reverseName).predicate(predicate)
  }  
...
export const relation =
  (
    ...
    relationClassVisitor?: Visitor
  ): Visitor =>
  (b) => {
    const relationClass = `${b.className}${target}Relation`
    if (Array.isArray(forward)) {
      forward = [forward[0] || _.camelCase(target), forward[1] || _.camelCase(target)]
    } else {
      forward = [forward || _.camelCase(target), _.camelCase(target)]
    }
    if (Array.isArray(backward)) {
      backward = [backward[0] || _.camelCase(b.className), backward[1] || _.camelCase(b.className)]
    } else {
