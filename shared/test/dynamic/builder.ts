...
describe('dynamic', () => {
  describe('builder', () => {
    describe('schema', () => {
      ...
      it('should define property index', () => {
        const s = schema()
        s.class('Book').property('title').index()
        const g = s.build()
        expect(g)
          .to.have.nested.property('graph.@graph.0.properties')
          .containSubset([{ name: 'title', index: true }])
      })
    ...
    describe('relation', () => {
      ...
      it('should define custom forward reverse property', () => {
        const s = schema()
        s.class('Author')
          .class('Book')
          .accept(relation('Author', 'writtenBy', [, 'work']))
        const g = s.build()
        expect(g)
          .to.have.nested.property('graph.@graph.1.properties')
          .containSubset([
      ...
      it('should define custom backward reverse property', () => {
        const s = schema()
        s.class('Author')
          .class('Book')
          .accept(relation('Author', [, 'person'], 'writes'))
        const g = s.build()
        expect(g)
          .to.have.nested.property('graph.@graph.0.properties')
          .containSubset([
      ...
      it('should define custom forward and backward reverse properties', () => {
        const s = schema()
        s.class('Author')
          .class('Book')
          .accept(relation('Author', ['writtenBy', 'person'], ['writes', 'work']))
        const g = s.build()
        expect(g)
          .to.have.nested.property('graph.@graph.1.properties')
          .containSubset([
      ...
      it('should define custom forward and backward properties', () => {
        const s = schema()
        s.class('Author')
          .class('Book')
          .accept(relation('Author', ['writtenBy'], ['writes']))
        const g = s.build()
        expect(g)
          .to.have.nested.property('graph.@graph.1.properties')
          .containSubset([
    ...
    describe('oneToMany', () => {
      it('should define one to many relation', () => {
        const s = schema()
        s.class('Author').accept(oneToMany('Book', 'theBook', 'theAuthor'))
        const g = s.build()
        expect(g)
          .to.have.nested.property('graph.@graph.0.properties')
          .containSubset([
            { name: 'theBook', reverse_name: 'theAuthor', multiplicity: 'multiple', reverseMultiplicity: 'single' },
          ])
      })
      it('should define one to many relation with default names', () => {
        const s = schema()
        s.class('Author').accept(oneToMany('Book'))
        const g = s.build()
        expect(g)
          .to.have.nested.property('graph.@graph.0.properties')
          .containSubset([
    ...
    describe('manyToMany', () => {
      ...
      it('should define many to many relation with custom predicate', () => {
        const s = schema()
        s.class('Author').accept(manyToMany('Book', 'theBook', 'theAuthor', 'http://example.com/author_books'))
        const g = s.build()
        expect(g)
          .to.have.nested.property('graph.@graph.0.properties')
          .containSubset([
